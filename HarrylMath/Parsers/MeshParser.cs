...
{
    public static unsafe class MeshParser
    {
                ...
                foreach (string line in lines)
                {
                    ...
                    switch (args[0])
                    {
                        case "usemtl":
                            ...
                            break;
                        case "v":
                            *vertices[vertIndex] = new Vector3(Convert.ToDouble(args[1]), Convert.ToDouble(args[2]), Convert.ToDouble(args[3]));
                            vertIndex++;
                            break;
                        case "vt":
                        ...
                        case "vn":
                            ...
                            break;
                        case "f":
                            int faceVertCount = args.Length - 1;
                            Vector3*[] faceVertices = new Vector3*[faceVertCount];
                            List<Vector2> faceTextures = new List<Vector2>();
                            List<Vector3> faceNormals = new List<Vector3>();
                            for (int i = 1; i < args.Length; i++)
                            {
                                string[] args1 = args[i].Split("/");
                                faceVertices[i - 1] = vertices[Int32.Parse(args1[0]) - 1];
        ...
        public static Mesh FromIcoSphere(Vector3 origin, double radius, int iterations = 2, NormalType normalType = NormalType.Default)
        {
            ...
            Vector3 a = new Vector3(goldenRatio, 1.0, 0.0).Normalized() * radius;
            Vector3 b = new Vector3(goldenRatio, -1.0, 0.0).Normalized() * radius;
            Vector3 c = new Vector3(-goldenRatio, -1.0, 0.0).Normalized() * radius;
            Vector3 d = new Vector3(-goldenRatio, 1.0, 0.0).Normalized() * radius;
            Vector3 e = new Vector3(1.0, 0.0, goldenRatio).Normalized() * radius;
            Vector3 f = new Vector3(-1.0, 0.0, goldenRatio).Normalized() * radius;
            Vector3 g = new Vector3(-1.0, 0.0, -goldenRatio).Normalized() * radius;
            Vector3 h = new Vector3(1.0, 0.0, -goldenRatio).Normalized() * radius;
            Vector3 i = new Vector3(0.0, goldenRatio, 1.0).Normalized() * radius;
            Vector3 j = new Vector3(0.0, goldenRatio, -1.0).Normalized() * radius;
            Vector3 k = new Vector3(0.0, -goldenRatio, -1.0).Normalized() * radius;
            Vector3 l = new Vector3(0.0, -goldenRatio, 1.0).Normalized() * radius;
            ...
            List<MeshTriangle3> Subdivide(List<MeshTriangle3> tris)
            {
                ...
                foreach (MeshTriangle3 triangle in tris)
                {
                    ...
                    Vector3 d = ((*a + *b) / 2).Normalized() * radius;
                    Vector3 e = ((*a + *c) / 2).Normalized() * radius;
                    Vector3 f = ((*b + *c) / 2).Normalized() * radius;
        ...
        {
            Vector3 a = new Vector3(1, 1, -1).Normalized() * radius;
            Vector3 b = new Vector3(1, 1, 1).Normalized() * radius;
            Vector3 c = new Vector3(-1, 1, -1).Normalized() * radius;
            Vector3 d = new Vector3(-1, 1, 1).Normalized() * radius;
            Vector3 e = new Vector3(1, -1, -1).Normalized() * radius;
            Vector3 f = new Vector3(1, -1, 1).Normalized() * radius;
            Vector3 g = new Vector3(-1, -1, -1).Normalized() * radius;
            Vector3 h = new Vector3(-1, -1, 1).Normalized() * radius;
            ...
            List<MeshQuad3> Subdivide(List<MeshQuad3> quads)
            {
                ...
                foreach (MeshQuad3 quad in quads)
                {
                    ...
                    Vector3* c = quad.c;
                    Vector3* d = quad.d;
                    Vector3 e = ((*a + *d) / 2.0).Normalized() * radius;
                    Vector3 f = ((*d + *c) / 2.0).Normalized() * radius;
                    Vector3 g = ((*c + *b) / 2.0).Normalized() * radius;
                    Vector3 h = ((*b + *a) / 2.0).Normalized() * radius;
                    Vector3 i = ((*a + *d + *c + *b) / 4.0).Normalized() * radius;
